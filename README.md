...
- **Licence & rating progress (for students)** — track cumulative progress toward a licence's minimum aeronautical experience, computed live from logged flights, with a progress bar per requirement. Only PPL — Aeroplane is built so far, with its figures taken from **CARs Standard 421.26(4)**: 45 hours total (at most 5 on a simulator/FTD, and the engine deducts any excess), 17 dual (incl. 3 cross-country and 5 instrument), and 12 solo (incl. 5 cross-country). Requirements that can't be derived from logged hours — notably the 150 nm solo cross-country with 2 full-stop landings away from the departure point — are surfaced as a manual checklist rather than silently omitted. Standards get amended, so confirm against the current published text before a flight test application. Night Rating, CPL, and Instrument Rating templates are natural next additions (`src/lib/licenseRequirements.ts`).
