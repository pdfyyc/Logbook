...
- **Licence & rating progress (for students)** — track cumulative progress toward a licence's minimum aeronautical experience, computed live from logged flights: total time, dual, solo, cross-country, and instrument hours against the requirement, with a progress bar per item. Only PPL — Aeroplane is built so far, and its hour requirements are recalled from memory rather than verified against the current Standard 421 text (the app says so, loudly, in the feature itself) — confirm with your flight school before relying on it for a flight test application. Night Rating, CPL, and Instrument Rating templates are natural next additions (`src/lib/licenseRequirements.ts`).
