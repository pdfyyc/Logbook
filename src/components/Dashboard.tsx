...
import type { Aircraft, Flight, ImportBatch, PilotProfile } from "../types";
import {
  computeCarsCurrency,
  computeMonthTotals,
  formatHours,
...
import { calculateFlightTotals } from "../lib/flightTotals";
...
export function Dashboard({
  ...
  const totalsResult = calculateFlightTotals(flights, aircraftById);
  const totals = totalsResult.totals;
  const multiEngine =
    totalsResult.byCategory.find(
      (group) => group.key === "multi-engine-aeroplane",
    )?.totals.totalTime ?? 0;
      ...
      {flights.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            <StatCard
              label="Total time"
              value={formatHours(totals.totalTime)}
              icon={Clock}
            />
            <StatCard label="PIC" value={formatHours(totals.pic)} icon={User} />
            <StatCard
              label="Cross-country"
              value={formatHours(totals.crossCountry)}
              icon={MoveRight}
            />
            <StatCard
              label="Night"
              value={formatHours(totals.night)}
              icon={Moon}
            />
            <StatCard
              label="Instrument"
            ...
            {totals.dualGiven > 0 && (
              <StatCard
                label="Instructor"
                value={formatHours(totals.dualGiven)}
                icon={User}
              />
            )}
          </div>
          ...
          >
            Open detailed reports and contributing flights
          </button>
            ...
            <div className="space-y-2">
              {recent.map((f) => (
                <FlightRow
                  key={f.id}
                  flight={f}
                  aircraft={aircraftById.get(f.aircraftId)}
                />
              ))}
            </div>
          </div>
      ...
      <Card className="p-4">
        <div className="mb-3 flex items-baseline justify-between gap-2">
          ...
          <span className="text-xs text-[var(--text-muted)]">
            Rolling windows, recalculated on every flight — for planning only,
            verify against the CARs
          </span>
        </div>
  ...
  return (
    <div className="rounded-lg border border-[var(--border)] p-3">
      <div className="mb-1 flex flex-wrap items-center gap-x-2 gap-y-1">
        <span
          className={`h-2 w-2 shrink-0 rounded-full ${levelDot[item.level]}`}
          aria-hidden
        />
      ...
      {item.explanation && (
        <p className="mt-1 text-[10px] text-[var(--text-muted)]">
          How calculated: {item.explanation}
        </p>
      )}
      {(item.earliestQualifyingDate || item.nextChangeDate) && (
        <p className="mt-1 text-[10px] text-[var(--text-muted)]">
          {item.earliestQualifyingDate
            ? `Earliest qualifying event: ${item.earliestQualifyingDate}. `
            : ""}
          {item.nextChangeDate
            ? `Next possible status change: ${item.nextChangeDate}.`
            : ""}
        </p>
      )}
      ...
      {item.qualifying.length > 0 && (
        <div className="mt-2">
          ...
          >
            ...
            {expanded ? "Hide" : "Show"} qualifying flights (
            {item.qualifying.length})
          </button>
          {expanded && (
                    ...
                    <button
                      ...
                      onClick={() => {
                        const flight = flights.find(
                          (entry) => entry.id === q.flightId,
                        );
                        if (flight) onEditFlight(flight);
                      }}
                    >
                      Review
                    </button>
