...
export function LicenseProgress({
  ...
  const untracked = LICENSE_TEMPLATES.filter(
    (t) => !trackedGoals.includes(t.id),
  );
    ...
    <Card className="p-4">
      <div className="mb-1 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-[var(--text)]">
          Licence & rating progress
        </h3>
      </div>
      <p className="mb-3 text-xs text-[var(--text-muted)]">
        Hour requirements are taken from the Transport Canada standard cited on
        each licence below. Standards are amended from time to time — confirm
        against the current published text before a flight test application.
      </p>
          ...
          return (
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] text-[var(--text-muted)]">
                      {template.sourceTitle} · {template.ruleVersion}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div key={item.id}>
                    <div className="mb-0.5 flex items-center justify-between gap-2 text-xs">
                      ...
                      <span className="font-mono text-[var(--text-muted)]">
                        {item.blockedReason
                          ? "—"
                          : formatAmount(item.have, item.unit)}{" "}
              ...
              <p className="mt-2 text-[10px] text-[var(--text-muted)]">
                * Approximated from flights logged as wholly solo or wholly dual
                — a mixed flight won't split correctly.
              </p>

              {template.manualRequirements.length > 0 && (
                <div className="mt-3 border-t border-[var(--border)] pt-2">
                  <p className="mb-1.5 text-[11px] font-medium text-[var(--text)]">
                  ...
                  <ul className="space-y-1.5">
                    {template.manualRequirements.map((req) => (
                      <li
                        key={req}
                        className="flex items-start gap-1.5 text-[11px] leading-snug text-[var(--text-muted)]"
                      >
      ...
      <p className="mt-3 text-[10px] text-[var(--text-muted)]">
        Aeroplane permits, licences and ratings only. Helicopter, glider and
        balloon requirements differ and aren't modelled here.
      </p>
    </Card>
  );
}
