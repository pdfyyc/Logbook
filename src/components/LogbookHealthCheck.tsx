...
export function LogbookHealthCheck({ flights, aircraftById, onReviewFlight, uncertainties = [] }: Props) {
  const issues = checkLogbookHealth(flights, aircraftById, uncertainties)
  return (
    <Card className="p-4">
        ...
        <ul className="mt-3 space-y-2">
          {issues.map((issue) => (
            <li key={issue.id} className="flex gap-2 rounded-lg border border-[var(--border)] p-2.5">
              <CircleAlert size={15} className={issue.level === "error" ? "mt-0.5 shrink-0 text-red-500" : issue.level === "uncertainty" ? "mt-0.5 shrink-0 text-sky-500" : "mt-0.5 shrink-0 text-amber-500"} />
              <div>
                <p className="text-xs font-medium text-[var(--text)]">{issue.title} <span className="ml-1 uppercase text-[10px] text-[var(--text-muted)]">{issue.level}</span></p>
